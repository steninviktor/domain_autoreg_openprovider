...
Во время периодической проверки GUI показывает таймер до следующего цикла и обновляет данные после завершения проверки.
...
Массовое удаление требует подтверждения во всплывающем окне.
...
Технический polling GUI не пишется в лог, чтобы файл не разрастался бессмысленно.
...
- GUI требует подтверждение во всплывающем окне перед запуском проверки при включенной боевой регистрации.
...
Такое уведомление отправляется только один раз для домена.
...
Перед коммитом и публикацией проверьте, что в git не попали:

- `.env`;
- `config.yaml`;
- `state/domains.sqlite3`;
- `domain-autoreg.log`;
- реальные `start_gui.bat` / `stop_gui.bat`, если в них есть локальные пути.
